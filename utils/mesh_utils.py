...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    if cluster_to_keep > len(cluster_n_triangles):
        ...
        print("Warning: cluster_to_keep is larger than the number of clusters, set to {}".format(cluster_to_keep))
...
class GaussianExtractor(object):
    def __init__(self, gaussians, gaussian_render):
        """
    ...
    @torch.no_grad()
    def reconstruction(self, training_cameras):
        """
        reconstruct radiance field given cameras
        """
        self.clean()
        images_count = training_cameras.dataset.list_cam_infos.__len__()
        print(f"Reconstructing the scene with {images_count} views")
        self.depthmaps = [None] * images_count
        self.rgbmaps = [None] * images_count
        self.viewpoint_stack = [None] * images_count
        reconstruction_progress_bar = tqdm(range(0, images_count), desc="extracting RGB and Depth...")
        for viewpoint_batch in training_cameras:
            for viewpoint_cam in viewpoint_batch:
                viewpoint_cam.world_view_transform = viewpoint_cam.world_view_transform.cuda()
                viewpoint_cam.projection_matrix = viewpoint_cam.projection_matrix.cuda()
                viewpoint_cam.full_proj_transform = viewpoint_cam.full_proj_transform.cuda()
                viewpoint_cam.camera_center = viewpoint_cam.camera_center.cuda()
                render_pkg = self.render(viewpoint_cam, self.gaussians, with_inv_depth = False)
                src_render_image, render_depth = render_pkg["render"], render_pkg["depth"]
                self.rgbmaps[viewpoint_cam.uid] = src_render_image.cpu()
                self.depthmaps[viewpoint_cam.uid] = render_depth.cpu()
                self.viewpoint_stack[viewpoint_cam.uid] = viewpoint_cam.shallow_clone()
