...
from scene.colmap_loader import read_points3D_binary, read_extrinsics_binary, qvec2rotmat
...
def get_Sphere_Norm(xyz):
    ...
    scale = 1.0 #cfg.data.get('sphere_scale', 1.0)
    radius *= scale
...
def get_obj_pose_tracking(datadir, selected_frames, ego_poses, cameras=[0, 1, 2, 3, 4]):
    ...
    box_scale = 1.0 #cfg.data.get('box_scale', 1.0)
...
def generate_dataparser_outputs(
    ...
    # run colmap
    colmap_basedir = os.path.join(f'{datadir}/camera_calibration/rectified')
    if not os.path.exists(os.path.join(colmap_basedir, 'sparse/0')):
