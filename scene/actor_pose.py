...
import numpy as np
from utils.general_utils import quaternion_raw_multiply, get_expon_lr_func, matrix_to_quaternion
...
class ActorPose(nn.Module):
    ...
    def training_setup(self):
        if True: #self.opt_track:
            params = [
                {'params': [self.opt_trans], 'lr': 0.005, 'name': 'opt_trans'},
                {'params': [self.opt_rots], 'lr': 0.001, 'name': 'opt_rots'},
            ]
            
            self.opt_trans_scheduler_args = get_expon_lr_func(lr_init=0.005,
                                                    lr_final=5e-5,
                                                    lr_delay_mult=0.01,
                                                    max_steps=30000,
                                                    )
            
            self.opt_rots_scheduler_args = get_expon_lr_func(lr_init=0.001,
                                                    lr_final=1e-5,
                                                    lr_delay_mult=0.01,
                                                    max_steps=30000,
                                                    )
    ...
    def get_tracking_translation(self, track_id, camera: Camera):
        if False: #self.opt_track and camera.metadata['is_val']:
            timestamp1, timestamp2 = self.find_closest_camera_timestamps(track_id, camera)
            if timestamp1 is None:
                return self.get_tracking_translation_(track_id, camera.metadata['timestamp'])
            else:
                timestamp = camera.metadata['timestamp']
    ...
    def get_tracking_rotation(self, track_id, camera: Camera):
        if False: # self.opt_track and camera.metadata['is_val']:
            timestamp1, timestamp2 = self.find_closest_camera_timestamps(track_id, camera)
            if timestamp1 is None:
                return self.get_tracking_rotation_(track_id, camera.metadata['timestamp'])
            else:
                timestamp = camera.metadata['timestamp']
